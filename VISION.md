...
| **LiteLLM / gateways** | Key/user/team max_budget on the proxy | A cautionary tale, not just a competitor: public issue tracker shows budget limiters instantiated but never registered, team-key bypasses, and regressions where spend continues past the cap — and at least one customer left over it. Proof of demand *and* proof that enforcement bolted onto a router as feature #47 doesn't get engineered to be trustworthy. |
