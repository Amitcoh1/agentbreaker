...
// `breakerbox update-prices` refreshes both; this test fails if only one was updated.
describe("dashboard price table matches the Python source", () => {
  it("is byte-for-byte the same table", () => {
    const py = JSON.parse(
      readFileSync(fileURLToPath(new URL("../../../src/breakerbox/prices.json", import.meta.url)), "utf8"),
    );
